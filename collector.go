...
var (
	metricMaps = map[string]map[string]ColumnMapping{
		...
		"stats": {
			...
			"avg_xact_count":    {COUNTER, "avg_xact_total", 1, "Average transactions per second in last stat period"},
			"avg_query_count":   {COUNTER, "avg_query_total", 1, "Average queries per second in last stat period"},
			"avg_recv":          {COUNTER, "avg_recv_bytes", 1, "Average received (from clients) bytes per second"},
			"avg_sent":          {COUNTER, "avg_sent_bytes", 1, "Average sent (to clients) bytes per second"},
			"avg_xact_time":     {COUNTER, "avg_xact_seconds", 1e-6, "Average transaction duration in seconds"},
			"avg_query_time":    {COUNTER, "avg_query_seconds", 1e-6, "Average query duration in seconds"},
			"avg_wait_time":     {COUNTER, "avg_wait_seconds", 1e-6, "Time spent by clients waiting for a server in seconds (average per second)"},
		},
		"pools": {
