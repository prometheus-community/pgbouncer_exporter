...
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	...
	db, err := getDB(e.conn)
	if err != nil {
		e.logger.Debug("error setting up DB connection", "err", err.Error())
		up = 0
		return
	}
	defer db.Close()
	...
	if err != nil {
		e.logger.Debug("error getting version", "err", err.Error())
		up = 0
	}

	if err = queryShowLists(ch, db, e.logger); err != nil {
		e.logger.Debug("error getting SHOW LISTS", "err", err.Error())
		up = 0
	}

	if err = queryShowConfig(ch, db, e.logger); err != nil {
		e.logger.Debug("error getting SHOW CONFIG", "err", err.Error())
		up = 0
	}

	errMap := queryNamespaceMappings(ch, db, e.metricMap, e.logger)
	if len(errMap) > 0 {
		e.logger.Debug("error querying namespace mappings", "err", errMap)
		up = 0
	}
