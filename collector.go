...
var (
	metricMaps = map[string]map[string]ColumnMapping{
		"databases": {
			"name":                {LABEL, "name", 1, "Name of configured database entry."},
			"host":                {LABEL, "host", 1, "Host pgbouncer connects to."},
		...
		"stats": {
			"database":          {LABEL, "database", 1, "Database name"},
			...
			"avg_xact_count":    {COUNTER, "avg_xact_count", 1, "Average transactions per second in last stat period"},
			"avg_query_count":   {COUNTER, "avg_query_count", 1, "Average queries per second in last stat period"},
			"avg_recv":          {COUNTER, "avg_recv", 1, "Average received (from clients) bytes per second"},
			"avg_sent":          {COUNTER, "avg_sent", 1, "Average sent (to clients) bytes per second"},
			"avg_xact_time":     {COUNTER, "avg_xact_time", 1e-6, "Average transaction duration in microseconds"},
			"avg_query_time":    {COUNTER, "avg_query_time", 1e-6, "Average query duration in microseconds"},
			"avg_wait_time":     {COUNTER, "avg_wait_time", 1e-6, "Time spent by clients waiting for a server in microseconds (average per second)"},
		},
		"pools": {
			"database":   {LABEL, "database", 1, "Database name pgbouncer connects to."},
			"user":       {LABEL, "user", 1, "User name"},
		...
		"lists": {
			"databases":     {GAUGE, "databases", 1, "Number of databases"},
			"users":         {GAUGE, "users", 1, "Number of users"},
			"pools":         {GAUGE, "pools", 1, "Number of pools"},
			"free_clients":  {GAUGE, "free_clients", 1, "Number of free clients"},
			"used_clients":  {GAUGE, "used_clients", 1, "Number of used clients"},
			"login_clients": {GAUGE, "login_clients", 1, "Number of clients in login state"},
			"free_servers":  {GAUGE, "free_servers", 1, "Number of free servers"},
			"used_servers":  {GAUGE, "used_servers", 1, "Number of used servers"},
			"dns_names":     {GAUGE, "dns_names", 1, "dns names"},
			"dns_zones":     {GAUGE, "dns_zones", 1, "dns zones"},
			"dns_queries":   {GAUGE, "dns_queries", 1, "dns queries"},
			"dns_pending":   {GAUGE, "dns_pending", 1, "dns pending"},
		},

		"mem": {
			"name":     {LABEL, "name", 1, "mem name"},
			"size":     {GAUGE, "size", 1, "mem size"},
			"used":     {GAUGE, "used", 1, "mem used"},
			"free":     {GAUGE, "free", 1, "mem free"},
			"memtotal": {GAUGE, "memtotal", 1, "mem total"},
		},
	}
