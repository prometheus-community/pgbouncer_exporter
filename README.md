...
stats.avg_xact_count | pgbouncer_stats_avg_xact_count | Average transactions per second in last stat period
stats.avg_query_count | pgbouncer_stats_avg_query_count | Average queries per second in last stat period
stats.avg_recv | pgbouncer_stats_avg_recv | Average received (from clients) bytes per second
stats.avg_sent | pgbouncer_stats_avg_sent | Average sent (to clients) bytes per second
stats.avg_xact_time | pgbouncer_stats_avg_xact_time | Average transaction duration in microseconds
stats.avg_query_time | pgbouncer_stats_avg_query_time | Average query duration in microseconds
stats.avg_wait_time | pgbouncer_stats_avg_wait_time | Time spent by clients waiting for a server in microseconds (average per second)
...
lists.databases | pgbouncer_lists_databases | Number of databases
lists.users | pgbouncer_lists_users | Number of users
lists.pools | pgbouncer_lists_pools | Number of pools
lists.free_clients | pgbouncer_lists_free_clients | Number of free clients
lists.used_clients | pgbouncer_lists_used_clients | Number of used clients
lists.login_clients | pgbouncer_lists_login_clients | Number of clients in login state
lists.free_servers | pgbouncer_lists_free_servers | Number of free servers
lists.used_servers | pgbouncer_lists_used_servers | Number of used servers
lists.dns_names | pgbouncer_lists_dns_names | dns names
lists.dns_zones | pgbouncer_lists_dns_zones | dns zones
lists.dns_queries | pgbouncer_lists_dns_queries | dns queries
lists.dns_pending | pgbouncer_lists_dns_pending | dns pending
mem.name | pgbouncer_mem_name | mem name
mem.size | pgbouncer_mem_size | mem size
mem.used | pgbouncer_mem_used | mem used
mem.free | pgbouncer_mem_free | mem free
mem.memtotal | pgbouncer_mem_memtotal | mem total